...
- **Connect** — link two shapes with a connector: pick one of the three connector shortcuts (**Straight**, **Elbow**, or **Curved**), then drag from a source shape to a target shape (a live preview follows the cursor and the target highlights). Connectors attach to shape **connection points** and **re-route automatically** when you move a shape; deleting a shape removes its connectors. Select one and switch its **Type** (Straight / Elbow / Curved) in the properties panel. Click to select, **Delete** to remove.
